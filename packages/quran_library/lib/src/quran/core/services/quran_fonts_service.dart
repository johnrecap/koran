...
class QuranFontsService {
  ...
  static List<int> _buildInteractiveLoadOrder(int centerPage, int radius) {
    final clampedCenter = centerPage.clamp(1, _totalPages);
    if (radius <= 0) {
      return <int>[clampedCenter];
    }

    return <int>[
      clampedCenter,
      for (int delta = 1; delta <= radius; delta++) ...[
        if (clampedCenter + delta <= _totalPages) clampedCenter + delta,
        if (clampedCenter - delta >= 1) clampedCenter - delta,
      ],
    ];
  }
  ...
  static Future<void> ensurePagesLoaded(
    ...
    final loadOrder = _buildInteractiveLoadOrder(centerPage, radius);

    for (var i = 0; i < loadOrder.length; i++) {
      final page = loadOrder[i];
