...
class QuranCtrl extends GetxController {
  ...
  void applyTajweedSetting(bool enabled) {
    state.isTajweedEnabled.value = enabled;
    GetStorage().write(_StorageConstants().isTajweed, enabled);
    update();
    update(['_pageViewBuild']);
  }
  ...
  void cancelScheduledQpcV4AllPagesPrebuild() {
    _qpcV4IdlePrebuildTimer?.cancel();
  }
  ...
  List<QpcV4RenderBlock> getQpcLayoutBlocksForPageSync(
    int pageNumber, {
    int neighborRadius = 2,
  }) {
    return getQpcV4BlocksForPageSync(
      pageNumber,
      neighborRadius: neighborRadius,
    );
  }
  ...
  Future<void> prewarmQpcV4Pages(
    ...
    final candidates = <int>[
      ...
      for (int offset = 1; offset <= neighborRadius; offset++) ...[
        if (basePage + offset <= 604) basePage + offset,
        if (basePage - offset >= 1) basePage - offset,
      ],
    ];
    ...
    for (final p in candidates) {
      ...
      didBuildAny = true;
      builtPages++;
      if (builtPages < candidates.length) {
