...
class QuranLibraryScreen extends StatelessWidget {
  ...
  /// Controls how aggressively page mode prepares nearby pages during flips.
    ...
    if (withPageView) {
      return Focus(
        ...
        child: PatchedPreloadPageView.builder(
          preloadPagesCount: pagingPerformanceProfile.pageViewPreloadCount,
          padEnds: false,
          itemCount: 604,
          ...
          itemBuilder: (ctx, index) => _ItemBuilderWidget(
            ...
            parentContext: parentContext,
            isFontsLocal: isFontsLocal,
            pagingPerformanceProfile: pagingPerformanceProfile,
          ),
        ),
      );
    }
  ...
  void _onPageChange(BuildContext context, int pageIndex, QuranCtrl quranCtrl) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (quranCtrl.state.fontsSelected.value == 0) {
        ...
        if (pagingPerformanceProfile.interactiveQpcPrewarmRadius > 0) {
          Future(() => quranCtrl.prewarmQpcV4Pages(
                pageIndex,
                neighborRadius:
                    pagingPerformanceProfile.interactiveQpcPrewarmRadius,
              ));
        }
        if (pagingPerformanceProfile.enableIdleFullPrebuild) {
          quranCtrl.scheduleQpcV4AllPagesPrebuild(
            delay: pagingPerformanceProfile.idleFullPrebuildDelay,
          );
        } else {
          quranCtrl.cancelScheduledQpcV4AllPagesPrebuild();
        }
...
class _ItemBuilderWidget extends StatelessWidget {
  const _ItemBuilderWidget({
    ...
    required this.enableMotionRaster,
